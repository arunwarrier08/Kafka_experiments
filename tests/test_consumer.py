...
def test_consume_messages_success(mock_kafka_consumer, mock_db_connection, mocker):
    ...
    # Setup mock message
    test_data = {"content": "test message", "timestamp": datetime.now().isoformat()}
    mock_message = type(
        "Message",
        (),
        {"value": lambda: json.dumps(test_data).encode("utf-8"), "error": lambda: None},
    )
    ...
    mock_cursor.execute.assert_called_with(
        expected_query, (test_data["content"], test_data["timestamp"])
    )
...
def test_consume_messages_error(mock_kafka_consumer, mock_db_connection, mocker):
    ...
    mock_message = type(
        "Message", (), {"error": lambda: "Kafka error", "value": lambda: None}
    )
